...
'''
not leetcode! using dijekstra to find the shortest path between X and Y
...
class Solution:
    ...
    def findCheapestPrice(self, n, flights, src, dst):
        ...
        routes = []
        while hq:
            weight, to, cf = heapq.heappop(hq) #cf = coming froms
            print (cf,"-->",to) 
            routes.append( (cf,to) )
...
if __name__=='__main__':
    ...
    s = Solution()
    price = s.findCheapestPrice(n,flights,src,dst)
    print ('price:=',price)
